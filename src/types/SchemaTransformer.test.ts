...
describe('default ValidationTransformers', () => {
  ...
  it('should apply string validtions', () => {
    ...
    expect(JSON.stringify(passedRegexp)).toBe(
      JSON.stringify(yup.string().matches(new RegExp('^[a-z]+$'), 'this is regexp')),
    );
...
describe('SchemaTransformer', () => {
  ...
  it('should be able to add a new transformer', () => {
    ...
    const stringSchema = transformer.getValidationTransformers['test'](
      { type: 'VALIDATION_REQUIRED', msg: 'test' },
      yup.number(),
    );
  ...
  it('should be able to transforma an input', () => {
    ...
    const fixtureschema = yup
      .number()
      .required('this is required')
      .min(2000, 'year should be at least 2000')
      .max(3000, 'year should be at least 3000');
